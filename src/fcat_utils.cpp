...
bool HexOrDecStrToNum(std::string& str, uint16_t& number)
{
  ...
  if (*end == '\0') {
    ...
    MSG("HexOrDecStrToNum could not parse string (%s)", str.c_str());
  }
...
{
  if (str.size() != 2) {
    MSG("TLC string: (%s) must be 2 chars. size: (%zu)", str.c_str(),
             str.size());
    return false;
  }

  if ((str[0] < 'A') || (str[0] > 'Z') || (str[1] < 'A') || (str[1] > 'Z')) {
    MSG("TLC string: (%s) must be upper chars [A-Z]", str.c_str());
    return false;
  }
  ...
  MSG("Converted TLC string: (%s) to U16: (0x%X)", str.c_str(), number);
  return true;
}
                                        ...
                                        std::string&         str)
{
  ...
  switch (type) {
    ...
    case JSD_SDO_DATA_U64:
      ...
      break;
    default:
      MSG("SDO data type: %d", static_cast<int>(type));
  }
  return data;
}
...
std::string jsd_sdo_request_type_to_string(jsd_sdo_req_type_t req_type)
{
  switch (req_type) {
    ...
    default:
      MSG("Invalid request type: %d", static_cast<int>(req_type));
  }
  return "INVALID";
}
...
std::string jsd_sdo_data_type_to_string(jsd_sdo_data_type_t data_type)
{
  switch (data_type) {
    ...
    case JSD_SDO_DATA_U64:
      ...
      break;
    default:
      MSG("Invalid data type: %d", static_cast<int>(data_type));
  }
  return "UNSPECIFIED";
}
